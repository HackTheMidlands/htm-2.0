...
export const MissionBlock = ({
    title, body, image, statistics,
}) => (
    <section className={style.mission} style={{ backgroundImage: `url('${WaveBackground}')` }}>
        <Grid fluid className={style.grid}>
            <Row>
                <Col xs={12} md={10} mdOffset={1} lg={5} lgOffset={0} className={style.textCol} style={{ position: 'relative' }}>
                    <img src={OrangeSquare} alt="Orange square" className={style.orangeSquare} />
                    <img src={GreenTriangle} alt="Green triangle" className={style.greenTriangle} />
                    <h2 className={style.title}>{ title }</h2>
                    <p className={style.body}>{ body }</p>
                </Col>
                <Col xs={12} md={10} mdOffset={1} lg={6} lgOffset={1} className={style.imageCol}>
                    <div className={style.image}>
                        <img src={GreenCircleLarge} alt="Green circle" className={style.greenCircleLarge} />
                        <img className={style.featureImage} src={image} alt="Mission image" />
                        <img src={DotGrid} alt="Dot grid overlay" className={style.dotGrid} />
                    </div>
                    <ul className={style.statistics}>
                        { statistics.map((stat) => (
                            <li key={stat} className={style.statisticItem}>{ stat }</li>
                        ))}
                    </ul>
                </Col>
            </Row>
...
MissionBlock.defaultProps = {
    ...
    image: EventImage,
    statistics: ['5 Annual Hackathons', '1,000 + Hackers Attended', '150+ Projects Created'],
};
